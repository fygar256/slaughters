...
HITMAX=1 # number of rhinoceroses to hit
...
def disp_scores():
    st.locate(0,0)
    st.color((0,255,0))
    st.putstr("THE RHINOCEROSSLAUGHTER ")
    st.color((255,255,0))
    st.putstr("HITS: ")
...
def main():
    ...
    st.setscreen("RHINOCEROSELAUGHTER")
    while(1):
        ...
        if h==HITMAX:
            while(not st.getkey('q')):
                st.color((0,255,0))
                st.locate(9,12)
                s="YOU HIT "+str(number_of_hits)+" RHINOCEROSES"
                st.putstr(s)
                st.color((255,0,0))
                st.locate(13,13)
        ...
        if invaded():
            while(not st.getkey('q')):
                st.color((0,255,0))
                st.locate(2,12)
                st.putstr("THE RHINOCEROSES HAVE INVADED")
                st.color((255,0,0))
                st.locate(15,13)
                st.putstr("GAME OVER")
                st.color((0,255,255))
