...
HITMAX=1 # number of leechs to hit
    ...
    while(1):
        ...
        if h==HITMAX:
            while(not st.getkey('q')):
                st.color((0,255,0))
                st.locate(13,12)
                s="YOU HIT "+str(number_of_hits)+" LEECHS"
                st.putstr(s)
                st.color((255,0,0))
                st.locate(13,13)
        ...
        if invaded():
            while(not st.getkey('q')):
                st.color((0,255,0))
                st.locate(8,12)
                st.putstr("THE LEECHS HAVE INVADED")
                st.color((255,0,0))
                st.locate(15,13)
                st.putstr("GAME OVER")
                st.color((0,255,255))
