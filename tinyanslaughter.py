...
HITMAX=1 # number of tinyan to hit
    ...
    while(1):
        ...
        if h==HITMAX:
            while(not st.getkey('q')):
                st.color((0,255,0))
                st.locate(13,12)
                s="YOU HIT "+str(number_of_hits)+" TINYAN"
                st.putstr(s)
                st.color((255,0,0))
                st.locate(13,13)
